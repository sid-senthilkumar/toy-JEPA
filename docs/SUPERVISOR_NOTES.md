...
This document is for the **human reviewer**. Run through this checklist against the code and results before signing off.
...
- [ ] **Two-term objective.** The training loss is `L_pred + lambda * SIGReg(Z)`. No reconstruction term, no contrastive term, no additional regularizers.
...
- [ ] **`results/STATUS.md` is present and current.** Pass/fail against each criterion in `docs/OBJECTIVE.md` is stated explicitly.
...
| EMA encoder, momentum encoder, stop-gradient, or frozen backbone in the code | Reverted to pre-LeWM JEPA recipe — non-compliant |
...
- `docs/CONSTRAINTS.md` — the fixed-vs-free contract.
